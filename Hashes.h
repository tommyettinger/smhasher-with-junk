...
inline void waterhash_test(const void * key, int len, uint32_t seed, void * out) {
	*(uint32_t*)out = waterhash(key, (uint32_t)len, (uint64_t)seed);
}
#include "wheathash.h"

inline void wheathash_test(const void * key, int len, uint32_t seed, void * out) {
	*(uint64_t*)out = wheathash(key, (uint32_t)len, (uint64_t)seed);
}
#include "woothash.h"
...
inline void woot32_test(const void * key, int len, uint32_t seed, void * out) {
	*(uint32_t*)out = woothash(key, (uint64_t)len, (uint64_t)seed);
}

//inline void woadhash_test(const void * key, int len, uint32_t seed, void * out) {
//	*(uint64_t*)out = woadhash(key, (uint64_t)len, (uint64_t)seed);
//}

void Hive(const void * key, int len, uint32_t seed, void * out);
void Wisp(const void * key, int len, uint32_t seed, void * out);
