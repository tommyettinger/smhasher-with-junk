...
static inline uint32_t waterhash(const void* key, size_t len, uint64_t seed){
    ...
    case 10: seed = _watermum(_waterr32(p) - seed, _waterr32(p + 4) ^ _waterp2) + _watermum(seed ^ _waterp5, _waterr16(p + 8) ^ _waterp3); break;
    case 11: seed = _watermum(_waterr32(p) - seed, _waterr32(p + 4) ^ _waterp2) + _watermum(seed ^ _waterr16(p + 8), (_waterr08(p + 10)) ^ _waterp3); break;
    case 12: seed = _watermum(_waterr32(p) - seed, _waterr32(p + 4) ^ _waterp2) + _watermum(seed ^ _waterr16(p + 8), (_waterr16(p + 10)) ^ _waterp4); break;
    case 13: seed = _watermum(_waterr32(p) - seed, _waterr32(p + 4) ^ _waterp0) + _watermum(seed ^ _waterr32(p + 8), _waterp5 + (_waterr08(p + 12))); break;
    case 14: seed = _watermum(_waterr32(p) - seed, _waterr32(p + 4) ^ _waterp0) + _watermum(seed ^ _waterr32(p + 8), _waterp5 + (_waterr16(p + 12))); break;
    case 15: seed = _watermum(_waterr32(p) - seed, _waterr32(p + 4) ^ _waterp0) + _watermum(seed ^ _waterr32(p + 8), _waterp5 + (_waterr16(p + 12) << 8 | _waterr08(p + 14))); break;
    }
    seed = (seed ^ len) * (_waterp0 ^ seed << 16);
    return (int)(seed ^ __rolq(seed, 31) ^ __rolq(seed, 19));
}
#endif
