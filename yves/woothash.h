...
#ifndef woothash_version_1
#define woothash_version_1
#include <stdint.h>
#include <math.h>
...
static inline uint64_t woothash(const void* key, uint64_t len, uint64_t seed) {
	...
	{
		a = (_wootr64(p     ) ^ a) * _wootp1; a = ROTL64(a, 23); a *= _wootp3;
		b = (_wootr64(p +  8) ^ b) * _wootp2; b = ROTL64(b, 25); b *= _wootp4;
		c = (_wootr64(p + 16) ^ c) * _wootp3; c = ROTL64(c, 29); c *= _wootp5;
...
//			_wootmum(_wootr64(p + 16) ^ _wootp3, _wootr64(p + 24) ^ _wootp4));
//	}
//	seed += _wootp5;
//	switch (len & 31) {
//	case	1:	seed = _wootmum(seed, _wootr08(p) ^ _wootp1);	break;
...
static inline uint32_t woot32(const void* key, uint64_t len, uint64_t seed) {
	const uint8_t *p = (const uint8_t*)key;
	uint64_t i, a = seed ^ _wootp4, b = ROTL64(seed, 17) ^ _wootp3, c = ROTL64(seed, 31) ^ _wootp2, d = ROTL64(seed, 47) ^ _wootp1;
	for (i = 0; i + 32 <= len; i += 32, p += 32)
	{
